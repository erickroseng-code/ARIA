...
const LiquidGlassBackground = ({ energy = 0 }: Props) => {
    ...
    useEffect(() => {
        ...
        const spawnParticle = (): Particle => {
            ...
            // Favor the inner core with squared random for non-linear distribution
            const distFactor = Math.pow(Math.random(), 1.5);
            const baseRadius = minR + distFactor * (maxR - minR);
            const layer = baseRadius < maxR * 0.3 ? 0 : baseRadius < maxR * 0.6 ? 1 : 2;

            // Inner particles orbit faster (Kepler-like)
            const baseSpeed = (0.0008 + Math.random() * 0.0015) * (2 - baseRadius / maxR);
            ...
            return {
                ...
                wobble: 1.5 + Math.random() * 6,
                wobbleSpeed: 0.4 + Math.random() * 1.2,
                tilt: (Math.random() - 0.5) * 0.5, // slight Y-axis tilt for 3D
            };
        };
        ...
        const draw = () => {
            ...
            // Fade — aumentado para reduzir ruído visual e melhorar legibilidade
            ctx.fillStyle = `rgba(8, 10, 18, ${0.18 + smoothEnergy * 0.04})`;
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                // Color — Jarvis blue/cyan palette (high contrast for dark background)
                const colorPhase = Math.sin(p.hueOffset + time * 0.15);
                let hue: number, sat: number, light: number;

                if (colorPhase > 0.3) {
                    hue = 200 + colorPhase * 15; // blue
                    sat = 60 + smoothEnergy * 30;
                    light = 65 + intensity * 30;
                } else if (colorPhase > -0.3) {
                    hue = 190; // cyan
                    sat = 50 + smoothEnergy * 25;
                    light = 70 + intensity * 25;
                } else {
                    hue = 220 + colorPhase * 20; // deep blue
                    sat = 55 + smoothEnergy * 30;
                    light = 60 + intensity * 30;
                }
                ...
                // Core dot
                const alpha = intensity * (0.65 + smoothEnergy * 0.35);
                ctx.beginPath();
                ctx.arc(x, y, currentSize, 0, Math.PI * 2);
                ctx.fillStyle = `hsla(${hue}, ${sat}%, ${Math.min(light + 15, 95)}%, ${Math.min(alpha, 0.95)})`;
                ctx.fill();
                ...
                if (intensity > 0.3) {
                    ...
                    ctx.fillStyle = `hsla(${hue}, ${sat + 10}%, ${light}%, ${intensity * (0.08 + smoothEnergy * 0.12)})`;
                    ctx.fill();
                }
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                for (let j = i + 1; j < positions.length; j++) {
                    ...
                    const lineAlpha = proximity * proximity * a.intensity * b.intensity * (0.10 + smoothEnergy * 0.15);
                    if (lineAlpha < 0.004) continue;

                    ctx.beginPath();
                    ctx.moveTo(a.x, a.y);
                    ctx.lineTo(b.x, b.y);
                    ctx.strokeStyle = `hsla(205, ${40 + smoothEnergy * 35}%, ${65 + proximity * 25}%, ${lineAlpha})`;
                    ctx.lineWidth = 0.3 + proximity * 0.6 + smoothEnergy * 0.5;
                    ctx.stroke();
                }
            }
            ...
            for (let i = 0; i < pulseCount; i++) {
                ...
                const pulseAlpha = (1 - pulseProgress) * (0.10 + smoothEnergy * 0.15);

                if (pulseAlpha > 0.005) {
                    ctx.beginPath();
                    ctx.arc(cx, cy, pulseRadius, 0, Math.PI * 2);
                    ctx.strokeStyle = `hsla(200, ${60 + smoothEnergy * 35}%, ${70 + smoothEnergy * 20}%, ${pulseAlpha})`;
                    ctx.lineWidth = 1.0 + smoothEnergy * 1.8;
                    ctx.stroke();
                }
            }
