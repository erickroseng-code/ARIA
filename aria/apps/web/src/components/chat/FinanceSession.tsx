    ...
    <div className="flex flex-col items-center pt-6 pb-2">
      <div className="relative" style={{ width: 220, height: 150 }}>
        <svg viewBox="0 0 200 135" className="w-full h-full overflow-visible">
          ...
          <path d={arcPathStroke(outerThinR, 0, 100)} fill="none" stroke="hsl(0, 0%, 93%)" strokeWidth="2" strokeLinecap="round" />
          {segments.filter(s => s.from < score).map((seg, i) => (
            <path key={`ot-${i}`} d={arcPathStroke(outerThinR, seg.from, Math.min(seg.to, score))} fill="none" stroke={seg.color} strokeWidth="2" opacity={0.5} />
          ))}

          <path d={sectorPath(mainOuterR, mainInnerR, 0, 100)} fill="hsl(0, 0%, 95%)" />
          {segments.map((seg, i) => (
            <path key={`m-${i}`} d={sectorPath(mainOuterR, mainInnerR, seg.from, seg.to)} fill={seg.color} />
          ))}
          <path d={sectorPath(mainOuterR, mainInnerR, 0, score)} fill="url(#hatchFine)" />
          ...
          <path d={arcPathStroke(innerThinR, 0, 100)} fill="none" stroke="hsl(0, 0%, 93%)" strokeWidth="1.5" strokeLinecap="round" />
          {segments.filter(s => s.from < score).map((seg, i) => (
            <path key={`it-${i}`} d={arcPathStroke(innerThinR, seg.from, Math.min(seg.to, score))} fill="none" stroke={seg.color} strokeWidth="1.5" opacity={0.4} />
          ))}
        </svg>

        <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ paddingTop: 56 }}>
          <span className="text-[28px] font-bold text-card-foreground leading-none tracking-tight">{score.toFixed(1)}%</span>
